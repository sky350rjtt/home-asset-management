...
### 5.1 リプレイス時の方針変更（決定済み・未着手）

2026-08-16、QR/NFCによる物理識別（[ARCHITECTURE.md §4.6](ARCHITECTURE.md#46-qrnfcによる物理識別リプレイス時に着手)参照）
の検討過程で、現行の「拠点コード＋カテゴリコード＋年＋連番」というID構造そのものを見直す方針が決定した。

- **現状の課題**：IDに拠点/カテゴリコードを埋め込んでいるため、資産を別の部屋へ移動しても
  ID文字列は追従しない（`LOC_CODE`列は独立して更新できるため実害はないが、人間がIDの文字面を
  見て現在地と誤解しうる）。
- **リプレイス時の対応**：資産IDは**UUID化**し、拠点コード等の「現在の属性」は台帳（DB）側の列として
  管理する（IDの生成ロジックに拠点/カテゴリコードを組み込まない）。`assignId()` の連番管理・桁あふれ
  検知といった仕組み自体は、UUID化により丸ごと不要になる見込み。
- **現行システム（GAS）は据え置き**：ID構造・採番ロジックとも変更しない。上記はリプレイス時に
  適用する方針の記録であり、今すぐの改修は行わない。
