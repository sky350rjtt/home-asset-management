...
### 4.5 詳細カードAI Q&A（`AssetQA.gs`）の既知の制約とリプレイス時の改善案

現状の実装（2026-08-16追加）は、`?p=v`の詳細カードから「この資産の取扱説明書について質問する」機能を
提供するが、GASの実行モデル上、以下2点は意図的に妥協した簡易実装であり、現行システムのままでは
深追いしない方針（コスト対効果が見合わないため）。**バックエンドをSupabase等にリプレイスする際、
以下の設計に置き換えることで解決する。**

- **制約1: 質問のたびにPDFを毎回まるごと再アップロードしている。**
  `AssetQA.ask()` は質問1回ごとに `Gemini.analyze()`（File API resumable upload → ACTIVE化待ち →
  解析 → 一時ファイル削除）を最初からフルで実行する。回答は都度PDFの実内容に基づく（キャッシュや
  ハルシネーションではない）という利点はあるが、質問のたびに数秒〜数十秒のアップロード待ちが発生し、
  Gemini APIの呼び出しコスト・トークン消費も毎回まるごとPDF分かかる。
- **制約2: 会話履歴を保持しない単発リクエスト方式。**
  質問と回答は1往復ごとに独立しており、「それで、フィルターはどこ？」のような文脈依存の追加質問はできない。
  UIも回答欄が1つで、質問のたびに上書きされる（チャット的に積み上がらない）。

**リプレイス時の推奨設計：「抽出は1回だけ、質問はテキストのみ」**

家電の取扱説明書は通常数十ページ程度で収まるため、本格的なチャンク分割＋ベクトル検索（RAG）は
オーバースペック。以下の軽量な設計で、精度を落とさずコストだけ削減できる。

1. **抽出（ファイル登録時に1回だけ）：** Gemini（マルチモーダル）に「このPDFの内容を、図解の内容も
   含めてすべて文章で詳細に書き起こして」と依頼する。単純なOCR/テキスト抽出だと図解・配置図・アイコンの
   意味が失われるため、**抽出自体もGeminiに読ませて言葉に変換させる**のが肝（例：「排水フィルターの位置を
   示す図」→「本体下部左側の丸いカバーの中」という文章に変換される）。1資産に説明書が複数ファイルある
   場合は、まとめて1つの首尾一貫したテキストに変換する。
2. **保存：** 抽出結果をSupabaseに保存する（例：`asset_manual_content(asset_id, extracted_text,
   source_file_hash, extracted_at)`）。`source_file_hash`（元PDFの内容ハッシュや更新日時）を持たせ、
   元ファイルが差し替えられたら再抽出が必要と検知できるようにする（キャッシュの無効化）。
3. **質問時：** 保存済みの抽出済みテキストを**そのまま全文**LLMに渡す（チャンク検索は挟まない）。
   PDFファイル自体は二度と読み直さないため、質問のたびの処理は軽いテキストのやり取りだけになる。
4. **会話履歴：** 別テーブル（例：`qa_messages(asset_id, session_id, role, content, created_at)`）に
   保存し、直近数ターンを一緒にLLMへ渡せば、自然に会話継続の体験になる。

**この方式が通用する範囲（境界線）：** 上記は「抽出済みテキスト全文を毎回渡しても許容できるサイズ」で
あることが前提。もし異常に長大な説明書（数百ページ級の業務用機器マニュアル等）が出てきた場合は、
全文を毎回渡すとコンテキスト上限・コストの問題が再燃するため、その時だけチャンク分割＋embedding＋
ベクトル検索（本格RAG）に切り替える。この考え方自体は、既存の資産検索（`IntelligentSearch.gs` が
remarksをembedding化して類似検索する仕組み、[§4.3](#43-ベクトル検索の直列フォールバック方式)参照）
と地続きであり、リプレイス時は「resumable upload」を「事前テキスト化＋（必要な場合のみ）ベクトル検索」
に置き換える、という整理になる。

### 4.6 QR/NFCによる物理識別（リプレイス時に着手）

2026-08-16、`?p=v`のViewerに`?id=資産ID`で直接その資産の詳細カードを開くディープリンク機能
（`index_viewer.html`の`window.onload`内、`deepLinkId`）を追加した。これを使い、家電本体に
QR/NFCを貼って「かざす→即詳細カードが開く」運用が構想として検討され、**設計方針としては採用が決定した**。

**採用が決まった設計（永続識別子リゾルバパターン）：**

物理シール（QR/NFC）には、実装先アプリのURLを直接刻まない。代わりに自分が所有するドメイン
（`daiki.me`）配下の中継ページ（GitHub Pages想定）を経由させ、中継ページが「現在の実装先URL」へ
`?id=`を引き継いで転送する。これにより、バックエンドが将来何度入れ替わっても（GAS→Supabase等）、
物理シール自体は貼り替え不要になる。DOI・PURLなど、永続識別子として確立されたパターンと同じ考え方。

**ただし、現行システムでは実装を見送る（2026-08-16決定）：**

- 資産IDのUUID化（[SPEC.md §5.1](SPEC.md#51-リプレイス時の方針変更決定済み未着手)参照）や
  台帳の保存基盤そのものがリプレイスで大きく変わる予定であり、その前に物理QR/NFC・中継ページを
  作り込んでも、リプレイス時の変更幅を考えると投資に見合わない。
- そのため、中継ページの実装・QRコードの一括生成・NFCタグへの書き込みは、**いずれもリプレイス
  着手時に本項の設計方針に沿って対応する**。現行システムのコード変更は不要（ディープリンク機能
  自体は既に実装済みで、そのまま動作する）。

### 4.7 外部AIレビュー（Copilot、2026-08-16実施）の裏取り結果と対応

Copilotによるコードレビューを受け、主張を実コードで検証した上で対応方針を決定した。

**対応済み（本セッションで修正）：**
- **APIキーのConfigシート・フォールバック廃止**：`Config.gs`は元々スクリプトプロパティを優先しつつ
  Configシートの値にフォールバックする実装だったが、`ALLOWED_EMAILS`で家族にWebアプリ閲覧を許可した
  以上、その家族は同一スプレッドシート（Configシート含む）の閲覧権限も必要になる（`USER_ACCESSING`
  配下ではDrive/Sheetsアクセスも本人の権限で行われるため）。フォールバックを撤去し、スクリプト
  プロパティのみに一本化した（未設定ならfail-fastでエラーにする）。
- **未定義フォルダのスキップを可視化**：`Scanner.gs`は`Masters`に無いフォルダ名のサブフォルダを
  `console.warn`のみでスキップしており、実行結果アラートには一切現れなかった（開発者以外気づけない）。
  スキップしたフォルダ名を集計し、最終アラートに明示するよう修正した。

**指摘のうち事実誤認だったもの：**
- Copilotは「6分の実行時間制約に対する中断復旧設計が不十分」と指摘したが、これは誤り。
  `Scanner._execute()`には既に240秒（4分）の実行時間ガードがあり、上限到達時は安全にループを抜けて
  「未処理のファイルは次回のスキャンで処理されます」とユーザーに明示している（[§4.1](#41-gasの実行時間api呼び出しレート制約への対応)参照）。
  レビュー時にこのファイル・本ドキュメントを読み切れていなかったと考えられる。**外部AIレビューの
  指摘はそのまま信用せず、必ずコードで裏取りしてから対応すること。**

**リプレイス時に対応する（現行システムでは見送り、2026-08-16決定）：**
- **自動テスト・lintの整備**：`package.json`に`test`/`lint`スクリプトが無く、回帰検知の仕組みが
  ゼロである点は事実。ただし個人・家族利用の現行規模でゼロから追加する優先度は低いと判断。
  バックエンドをリプレイスする際、新しい基盤（Supabase等、通常のCI/CDが組みやすい環境）に
  移行するタイミングで、テストスイート・lintを新規に整備する。
- **プロンプトインジェクション対策**：説明書PDF等をAIにそのまま読ませる構成上、悪意ある文書に
  埋め込まれた指示によってAIの抽出結果や[AI Q&A回答](#45-詳細カードai-qaassetqags の既知の制約とリプレイス時の改善案)
  が誤誘導されるリスクは一般論として存在する。ただし本アプリのAI出力は台帳セルへの書き込みや
  質問応答の表示に留まり、自律的な危険操作（外部送信・削除等）には繋がらないため、実害は限定的と
  判断し、現行システムでは対応しない。リプレイス時、AIへの入力を信頼境界の外として扱う設計
  （出力のサニタイズ、想定外の指示混入を検知するガード等）を検討する。
