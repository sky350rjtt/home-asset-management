...
**リプレイス時の推奨設計（RAG方式への置き換え）：**

1. 説明書PDFのテキスト抽出・チャンク分割・embedding化を、**ファイルのアップロード時に1回だけ**実行し、
   Supabase（pgvector等）にチャンク単位で保存する。
2. 質問が来たら、質問文だけをembeddingして関連チャンクをベクトル検索し、そのテキスト片（数百〜千トークン
   程度）だけをLLMに渡す。PDFファイル自体を毎回送る必要がなくなり、速度・コストとも大幅に改善する。
3. 会話履歴も別テーブルに保存し、直近数ターンを一緒にLLMへ渡せば、自然に会話継続の体験になる。

この考え方自体は、既存の資産検索（`IntelligentSearch.gs` がremarksをembedding化して類似検索する仕組み、
[§4.3](#43-ベクトル検索の直列フォールバック方式)参照）と地続きであり、リプレイス時は「resumable
upload」を「事前チャンク化＋ベクトル検索」に置き換える、という整理になる。
